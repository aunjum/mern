// import CloseIcon from "@mui/icons-material/Close";
import { Button, Stack } from "@mui/material";
import Box from "@mui/material/Box";
import FormControl from "@mui/material/FormControl";
import Modal from "@mui/material/Modal";
import TextField from "@mui/material/TextField";
import { Formik } from "formik";
// import { useRouter } from "next/router";
import { useState ,useEffect, useContext, useRef, useCallback} from "react";
import styles from "./RecordRegistrationForm.module.css";
import CloseIcon from '@mui/icons-material/Close';
import InputLabel from '@mui/material/InputLabel';
import MenuItem from '@mui/material/MenuItem';
import Select from '@mui/material/Select';
import TextareaAutosize from '@mui/material/TextareaAutosize';
import { maxWidth } from "@mui/system";
import Grid from '@mui/material/Grid';
import Webcam from "react-webcam";
import FetchProcessingTypeApiHandler from "../../TrackBoardApi/FetchProcessingTypeApiHandler";
import CreateRecordApiHandler from "../../TrackBoardApi/CreateRecordApiHandler";



const style = {
  position: "absolute",
  top: "50%",
  left: "50%",
  transform: "translate(-50%, -50%)",
  width: 400,
  bgcolor: "background.paper",
  border: "2px solid #000",
  boxShadow: 24,
  p: 4,
};

const RecordRegistrationForm = (props) => {
   // const {tokenValue} = useContext(TokenContext);
   // console.log("group", tokenValue)
//   const { locale } = useRouter();
  const [open, setOpen] = useState(false);
  const [isValid, setIsValid] = useState(false);
  const handleOpen = () => setOpen(true);
  const [isProjectCreate, setIsProjectCreate] = useState(false);
  const [isError, setIsError] = useState(false);
  const [status,setStatus] = useState(false);
  const [allGroupData, setAllGroupData] = useState([]);
  const webcamRef = useRef(null);
  const [image, setImage] = useState("");
  const [processingTypeData, setProcessingTypeData] =  useState([]);
  const placeHolder = "data:image/jpeg;base64,";
  const handleClose = () => {
    setIsProjectCreate(false);
    setIsError(false);
    setOpen(false);
    //formik form reset
    // Formik.resetForm();

  };
  console.log("image " + image.split(',')[1]);
  console.log("imageData " + image);
  
  function dataURLtoFile(dataurl, filename) {
    var arr = dataurl.split(','), mime = arr[0].match(/:(.*?);/)[1],
        bstr = atob(arr[1]), n = bstr.length, u8arr = new Uint8Array(n);
        while(n--){
            u8arr[n] = bstr.charCodeAt(n);
        }
        return new File([u8arr], filename, {type:mime});
    }

    

  const initialValues = {
    processingType: "",
    location: "",
    toolUsed: "",
    comment: "",
  };

  const validate = (values) => {
    setIsValid(true);
    let errors = {};
    if (!values.processingType) {
      errors.processingType = "Processing Type is required";
      setIsValid(false);
    }

    if (!values.location) {
      errors.location = "Location is required";
      setIsValid(false);
    }
    if (!values.toolUsed) {
      errors.toolUsed = "Used Tool is required";
      setIsValid(false);
    }
    // if (!values.image) {
    //   errors.image = "Image is required";
    //   setIsValid(false);
    // }


    return errors;
  };

  // const loginFormSubmit = (e) => {
  //   e.preventDefault()
  //   router.push('/home')
  // }

  useEffect( () =>{
    console.log('device id', props.id);
    const FetchProcessingTypeData = async() =>{
      const allData = await FetchProcessingTypeApiHandler(props.id); 
      console.log(allData.body);
       if(allData.success === true)
       {
        setProcessingTypeData(await allData.body.request_type.processing_types);
        console.log(allData.body.request_type.processing_types)
           
       }
          
    }
    FetchProcessingTypeData();
  },[props.id])

  const onSubmit = async (values, { resetForm }) => {
    console.log(" record value",values);
    setStatus(true);
    const imageFile = dataURLtoFile(image, "img.jpg");
    console.log("aita e file",imageFile)
    const res = await CreateRecordApiHandler(values, props.id, imageFile);
    console.log("create record",res);
    if(res.success === true)
    {
      setIsProjectCreate(true);
      setIsError(false);
      resetForm();
    }
    else{
      setIsError(true);
      setIsProjectCreate(false);
    }

   
  };

  const videoConstraints = {
    width: 300,
    height: 300,
    facingMode: "user"
};
const capture = useCallback(
    () => {
        const imageSrc = webcamRef.current.getScreenshot();
        setImage(String(imageSrc));
        console.log(imageSrc);
    },
    [webcamRef]
);

const clear = useCallback(
    () => {
        setImage("");
    }
);



  return (
    <>
    
    <Formik
      initialValues={initialValues}
      onSubmit={onSubmit}
      validate={validate}
    >
      {(formik) => {
        return (
          <>
            {/* <Modal
              open={open}
              onClose={handleClose}
              aria-labelledby="modal-modal-title"
              aria-describedby="modal-modal-description"
            > */}
              {/* <Box sx={style}>
                <div className={styles.closeButton}>
                  <Button onClick={handleClose}>
                    <CloseIcon />
                  </Button>
                </div> */}
                 {isProjectCreate && (
                  <div className={styles["password-reset-email-success"]}>
                    Device Record Register successfully
                  </div>
                )}
                {isError && (
                  <div className={styles["password-reset-email-error"]}>
                    Device Record already exists
                  </div>
                )}
                
                <h2 className={styles['group-header-center']}>Device Record Registration </h2>
                <Box sx={{ flexGrow: 1 }}>
                  <Grid container spacing={2}>
                    <Grid xs={4}>  
                 
                <div className={styles["login-input-field"]}>
                  
                  <FormControl required className={styles["login-form"]} fullWidth>
                    <InputLabel id="organization-initial-label">Enter Processing Type</InputLabel>
                        {/* {formik.values.organizationInitial ? setOrganizationInitialName(formik.values.organizationInitial): null} */}
                        <Select
                              labelId="demo-simple-select-label"
                              id="processingType"
                              name="processingType"
                              value={formik.values.processingType}
                              label= "Enter Processing Type"
                              size="small"
                              onChange={formik.handleChange}
                              onBlur={formik.handleBlur}
                              error={
                                formik.touched.processingType &&
                                Boolean(formik.errors.processingType)
                              }
                            >
                            
                              {processingTypeData && processingTypeData.map((data) => (
                                <MenuItem
                                  key={data._id}
                                  value={data._id}
                                >
                                  {data.title_en}
                                </MenuItem>
                              ))}
                              {/* <MenuItem
                              key={10}
                              value={'10'} 
                              >
                              Processing Type 1
                              </MenuItem>
                              <MenuItem
                              key={20}
                              value={'20'} 
                              >
                              Procesing Type 2
                              </MenuItem> */}
                              
                            </Select>
                            <span className={styles["error"]}>
                              {formik.touched.processingType &&
                              formik.errors.processingType ? (
                                <div>{formik.errors.processingType}</div>
                              ) : null}
                            </span>
                        
                

                    <br />
                  </FormControl>
                  
                  <FormControl className={styles["login-form"]} fullWidth>
                    <TextField
                      id="location"
                      name="location"
                      label= "Enter Location"
                      variant="outlined"
                      size="small"
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                      value={formik.values.location}
                      error={
                        formik.touched.location &&
                        Boolean(formik.errors.location)
                      }
                      type="text"
                    />
                    <span className={styles["error"]}>
                      {formik.touched.location && formik.errors.location ? (
                        <div>{formik.errors.location}</div>
                      ) : null}
                    </span>
                    <br />
                  </FormControl>
                  <FormControl className={styles["login-form"]} fullWidth>
                    <TextField
                      id="toolUsed"
                      name="toolUsed"
                      label= "Enter Used Tool"
                      variant="outlined"
                      size="small"
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                      value={formik.values.toolUsed}
                      error={
                        formik.touched.toolUsed &&
                        Boolean(formik.errors.toolUsed)
                      }
                      type="text"
                    />
                    <span className={styles["error"]}>
                      {formik.touched.toolUsed && formik.errors.toolUsed ? (
                        <div>{formik.errors.toolUsed}</div>
                      ) : null}
                    </span>
                    <br />
                  </FormControl>
                  <FormControl className={styles["login-form"]} fullWidth>
                <div >
                  <TextareaAutosize
                    id="comment"
                    name="comment"
                    className="input"
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    value={formik.values.comment}
                    placeholder="Enter Comment"
                    style={{ width: '99%', maxWidth: '100%',minWidth:'50%', height:140, maxHeight:150 , minHeight:'50%', fontSize:'17px', fontWeight:"none", textIndent:'10px',border:'1px solid gray'}}
                  />
                </div>
              
              <br />
                  </FormControl>
                  
                </div>
                </Grid>
                <Grid item xs={8}>
                <Grid container spacing={2}>
                                <Grid item xs={6}>
                                <Webcam
                                        audio={false}
                                        height={300}
                                        ref={webcamRef}
                                        screenshotFormat="image/jpeg"
                                        width={300}
                                        videoConstraints={videoConstraints}
                                        mirrored={true}
                                    />
                                    {/* <span className={styles["error"]}>
                                      {formik.touched.image && formik.errors.image ? (
                                         <div>{formik.errors.image}</div>
                                          ) : null}
                                    </span> */}
                                    <br />
                                    <Button
                                    style={{
                                        color: 'white',
                                        backgroundColor: 'rgb(60, 179, 113)',
                                        fontSize: '10px',
                                        padding: '10px',
                                        border: '2px',
                                        borderRadius: '5px',
                                        width: '300px',
                                        wordWrap: 'break-word'
        
                                      }}
                                                type="submit"
                                                variant="contained"
                                                size="small"
                                                onClick={
                                                    capture
                                                }
                                            >
                                                Capture photo
                                            </Button>
                                    </Grid>
                                    <Grid item xs={6}>
                                    {image ? <img src={`${image}`} alt='record'/> : <img src={`${placeHolder}`} width={300} height={300} alt="record"/>}
                                    
                                    
                                    <span className={styles["error"]}>
                                           {image === "" ? (
                                             <div>Image is required</div>
                                                ) : null}
                                    </span>
                                   
                                    <br />
                                    <Button
                                    style={{
                                        color: 'white',
                                        backgroundColor: 'violet',
                                        fontSize: '12px',
                                        padding: '10px',
                                        border: '2px',
                                        borderRadius: '5px',
                                        width: '300px',
                                        wordWrap: 'break-word'
        
                                      }}
                                                type="submit"
                                                variant="contained"
                                                size="small"
                                                onClick={
                                                    clear
                                                }
                                            >
                                                Clear photo
                                            </Button>
                                    </Grid>
                                    </Grid>
                                    <Grid sx={12} spacing={2}>
                                
                                    </Grid>
                 </Grid>
        
               </Grid>
             </Box>
             <br/>
                <Stack justifyContent="center">
                  <Button
                    type="Submit"
                    variant="contained"
                    style={{
                        color: 'white',
                        backgroundColor: 'blue',
                        marginLeft: '45%',
                        fontSize: '12px',
                        padding: '10px',
                        border: '2px',
                        borderRadius: '5px',
                        width: '250px',
                        wordWrap: 'break-word'

                      }}
                    
                    onClick={formik.handleSubmit}
                   
                  >
                   Register
                  </Button>
                </Stack>
              
          </>
        );
      }}
    </Formik>
    </>
  );
};

export default RecordRegistrationForm;
